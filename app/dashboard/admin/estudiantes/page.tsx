...
function InfraccionDialog({
  selected,
  onClose,
}: {
  selected: Infraccion | null
  onClose: () => void
}) {
...
function EstudianteDetalleDialog({
  ...
  const retrasos = estudiante ? getRetrasoCount(estudiante.id) : 0
  const graves = infracciones.filter(
    (i) => i.tipo_falta?.gravedad === "grave" || i.tipo_falta?.gravedad === "muy_grave"
  ).length
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {g && (
                              <Badge variant="outline" className={`text-xs shrink-0 ${g.className}`}>
                                {g.label}
                              </Badge>
                            )}
                          </div>
...
export default function AdminEstudiantesPage() {
  ...
  const filtered = useMemo(() => {
    return mockEstudiantes.filter((e) => {
      ...
      const matchActivo =
        filterActivo === "all" ||
        (filterActivo === "activos" && e.activo) ||
        (filterActivo === "inactivos" && !e.activo)
      return matchSearch && matchCurso && matchSeccion && matchActivo
    })
  }, [search, filterCurso, filterSeccion, filterActivo])
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {mockEstudiantes.filter((e) => e.activo).length} estudiantes activos en el sistema
        </p>
      </div>
                        ...
                        <TableCell className="hidden lg:table-cell text-center">
                          {retrasos > 0 ? (
                            <div className="flex items-center justify-center gap-1">
                              ...
                              <span className="text-sm font-medium text-amber-600">{retrasos}</span>
                            </div>
                          ) : (
